# -*- coding: utf-8 -*-
"""
Data extraction sports-reference.com (https://www.sports-reference.com/cfb/)
and college football data package (https://github.com/CFBD/cfbd-python)
"""
import requests
    ...
    if year_list:
        for year in year_list:
            ...
            if exists(join(getcwd(),'all_data_regressor.csv')):
                all_data = read_csv(join(getcwd(),'all_data_regressor.csv'))  
            all_data = concat([all_data, final_data.dropna()])
            if not exists(join(getcwd(),'all_data_regressor.csv')):
                all_data.to_csv(join(getcwd(),'all_data_regressor.csv'))
            all_data.to_csv(join(getcwd(),'all_data_regressor.csv'))
